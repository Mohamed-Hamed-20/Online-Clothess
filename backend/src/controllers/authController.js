...
export const registerController = asyncHandler(async (req, res, next) => {
  ...
  //exisiting user
  if (exisitingUser) {
    return next(new Error("Already Register please login", { cause: 200 }));
  }
  ...
  const link = `${req.protocol}://${req.headers.host}/api/v1/auth/confirmEmail/${activationCode}`;
...
export const confirmEmail = asyncHandler(async (req, res, next) => {
  ...
  if (!activationCode) {
    return next(new Error("activation code is required "));
  }
  // check user
  const user = await userModel.findOne({ activationCode });
  if (!user?.activationCode) {
    return next(new Error("Invalid Activation code", { cause: 404 }));
  }
  //update
  user.isconfrimed = true;
  await user.save();
  //response
  return res.status(200).json({
    message: `${user.name} confirmed`,
    success: true,
...
export const loginController = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(new Error("Email is not registerd", { cause: 200 }));
  }
  if (!user.isconfrimed) {
    const activationCode = crypto.randomBytes(64).toString("hex");
    const link = `${req.protocol}://${req.headers.host}/api/v1/auth/confirmEmail/${activationCode}`;
    ...
    if (!isSend) {
      return next(new Error("something went wrong", { cause: 200 }));
    }
    return next(
      new Error("Email not confirmed check your inbox", { cause: 200 })
    );
  }
  const match = await comparePassword(password, user.password);
  if (!match) {
    return next(new Error("Invalid Password", { cause: 200 }));
  }
  //token
